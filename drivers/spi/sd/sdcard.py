...
Example usage on atsamd21:

    import machine, sdcard, os
    spi = machine.SPI(MOSI=machine.Pin.board.MOSI,
                      MISO=machine.Pin.board.MISO,
                      clock=machine.Pin.board.SCK)
    sd = sdcard.SDCard(spi, machine.Pin.board.SD)
    os.mount(sd, '/sd')
    os.listdir('/')
    os.listdir()
...
class SDCard:
    ...
    def init_card_v2(self):
        for i in range(_CMD_TIMEOUT):
            ...
            self.cmd(55, 0, 0)
            try:
                if self.cmd(41, 0x40000000, 0) == 0:        # 0x40000000 is a long int
                    self.cmd(58, 0, 0, 4)                  # 0x4000000  is a short int
                    self.cdv = 1
                    #print("[SDCard] v2 card")
                    return
            except OverflowError:
                if self.cmd(41, 0x4000000, 0) == 0:        # 0x40000000 is a long int
                    self.cmd(58, 0, 0, 4)                  # 0x4000000  is a short int
                    self.cdv = 1
                    #print("[SDCard] v2 card")
                    return
